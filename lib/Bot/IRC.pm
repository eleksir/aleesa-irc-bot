...
sub split_irc_msg {
    ...
    # as per https://www.ietf.org/rfc/rfc1459.txt#:~:text=IRC%20messages%20are%20always%20lines,the%20command%20and%20its%20parameters
    # we must split string into array of 512 characters including \r\n at the end of the string, in case of utf-8
    my $MAXLEN = 510;
    my @ret;
    my $prefix = '';

    my @message = split /:/, $str, 3;
    $str = pop @message;
    ...
    {
        do {
            my $buflen = bytes::length ($str);

            if ($buflen <= int($MAXLEN)) {
                push @ret, sprintf ("%s%s", $prefix, $str);
                last;
            }

            for (my $len = $MAXLEN; $len > 2; $len--) {
                my $buf = substr ($str, 0, $len);
                my $strip_offset = length $buf;

                while (substr($buf, 0, 1) eq ' ') {
                    $buf = substr($buf, 1);
                }

                while (substr ($buf, -1, 1) eq ' ') {
                    chop $buf;
                }

                if (bytes::length($buf) <= $MAXLEN) {
                    push @ret, sprintf ("%s%s", $prefix, $buf);
                    $str = substr ($str, $strip_offset);
                    last;
                }
            }
        } while (1);
